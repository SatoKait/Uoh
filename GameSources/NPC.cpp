...
namespace basecross {
	...
	void NPC::OnUpdate()
	{
		...
		auto stage = GetStage();
		auto changeFlag = stage->GetSharedGameObject<OpeningCameraman>(L"OpeningCameraman")->m_ChangeFlag;
		if (changeFlag && m_OnceFlag1)
		{
			m_OnceFlag1 = false;
			//アクション開始
			m_ptrAction->Run();
	...
	void NPC::OnCollisionEnter(shared_ptr<GameObject>& other)
	{
		...
		auto playercicle = stage->GetSharedGameObject<Player>(L"Player");
